      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/50">
            <TableHead className="sticky left-0 bg-muted/50 z-10 min-w-[200px] border-r">Data Siswa</TableHead>
            {MONTHS_LIST.map((month) => (
              <TableHead key={month} className="text-center min-w-[100px]">{month.substring(0, 3)}</TableHead>
            ))}
            <TableHead className="text-right sticky right-0 bg-muted/50 z-10 border-l">Aksi</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {students.length === 0 ? (
            ...
            students.map((student) => (
              <TableRow key={student.id} className="group hover:bg-muted/30">
                <TableCell className="sticky left-0 bg-background font-medium border-r shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                  ...
                  return (
                    <TableCell key={month} className="text-center p-0">
                      <button 
                        ...
                        className="w-full h-12 flex items-center justify-center transition-all hover:bg-accent/50"
                        title={`${month}: ${status}`}
                      >
                        {status === "PAID" ? (
                          ...
                          <Clock className="h-4 w-4 text-muted-foreground/20 group-hover:text-primary/40 transition-colors" />
                        )}
                      </button>
                    </TableCell>
                  )
                ...
                <TableCell className="text-right sticky right-0 bg-background border-l shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuLabel>Opsi Murid</DropdownMenuLabel>
